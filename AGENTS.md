...
- **Service layer**: `src/services/` — business logic (task-service, task-event-service, task-link-service, reminder-service, notification-service)
- **Access control**: `src/lib/auth/access-control.ts` — defines resources, actions, roles, and `resolvePermissions()`. The `permissions` JSON column on user stores per-user overrides for `"custom"` role.
- **Env vars**: `src/env.ts` using `@t3-oss/env-core` — client vars need `VITE_` prefix
...
## Gotchas
- `bunfig.toml` sets `frozenLockfile = true` and `install.auto = "disable"` — `bun install` won't auto-install missing packages and will fail on lockfile drift
